      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex items-center cursor-pointer" onClick={() => setPage('home')}>
            <img src="https://raw.githubusercontent.com/username/repo/main/logo.png" alt="BC博球" className="h-10 md:h-12 w-auto brightness-110" onError={(e) => {
              e.currentTarget.style.display = 'none';
              const fallback = e.currentTarget.nextElementSibling as HTMLElement;
              if (fallback) fallback.style.display = 'block';
            }} />
            <span className="hidden text-2xl font-black gold-text tracking-tighter italic">BC博球</span>
          </div>
          
          <div className="hidden md:flex items-center space-x-12">
            <button onClick={() => setPage('home')} className={`text-sm font-black tracking-widest hover:text-gold transition-colors ${currentPage === 'home' ? 'text-gold' : 'text-slate-400'}`}>首頁</button>
            ...
            <div className="group relative">
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                <div className="bg-[#0a0f1e] border border-white/10 p-2 rounded-2xl w-56 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
                  ...
                  <button onClick={() => setPage('ares-strategy')} className="w-full text-left px-5 py-4 text-sm font-bold text-slate-400 hover:text-gold hover:bg-white/5 rounded-xl transition-all">戰神賽特爆分法</button>
            ...
            <button onClick={() => onSectionNav('comparison')} className="text-sm font-black tracking-widest text-slate-400 hover:text-gold transition-colors">平台優勢</button>
